     ...
     <form method="post" action="{{ route('produk.store') }}">
       ...
       <div class="mb-6">
         ...
         <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
           <input type="text" placeholder="Nama Barang" class="border px-3 py-2 rounded"  name="nama" >
           <input type="text" placeholder="Harga" class="border px-3 py-2 rounded"   name="harga">
           <input type="text" placeholder="Kategori" class="border px-3 py-2 rounded md:col-span-2"  name="kategori">
           <textarea placeholder="Deskripsi" class="border px-3 py-2 rounded md:col-span-2" rows="3"  name="deskripsi"></textarea>
         </div>
          ...
          @foreach($items as $item)
            <tr class="border-t">
                ...
                <td class="p-2 space-x-2">
                  ...
                  <form action="{{ route('produk.destroy', $item->id) }}" method="POST" style="display:inline;" onsubmit="return confirm('Apakah Anda yakin ingin menghapus barang ini?');">
                @csrf
                @method('DELETE')
                <button type="submit" class="bg-red-600 text-white px-3 py-1 rounded hover:bg-red-700">Delete</button>
            </form>
                  <!-- <button class="bg-red-600 px-3 py-1 rounded text-white hover:bg-red-700"  onclick="konfirmasiHapus();">Hapus</button> -->
                </td>
              </tr>
        @endforeach
